...
### Changed
- Console workspace: Water-light remains only the outer edge. Interior rooms sit on a wide white canvas instead of an A4-width card. The in-page Metis mark is the geometric lockup on white (no black crop), with more space under the logo before room copy. Room pages drop the extra lead/next/statement paragraphs; one short task line remains. Protocol v2.9 copy law (short task heading, via-negativa only in help). `publish()` remains G2-BLOCKED.
