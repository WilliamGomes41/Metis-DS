...
def test_v26_delta_exists_and_is_the_live_baseline() -> None:
    delta = _read(DELTA)
    ...
    assert root_protocol.count("De geldende normatieve baseline is Protocol v2.6.0") == 1
    assert "De geldende normatieve baseline is Protocol v2.5.0" not in root_protocol
    assert "**Geldend protocol:** v2.6.0" in handoff
    assert "Protocol v2.6.0" in roadmap


def test_v26_hierarchy_points_at_one_live_version() -> None:
    root_protocol = _read(ROOT / "PROTOCOL.md")
    delta = _read(DELTA)
    assert root_protocol.count("De geldende normatieve baseline is Protocol v2.6.0") == 1
