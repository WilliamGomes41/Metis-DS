...
def registry(*, docs=(DOC,), scopes=("retrieve","knowledge:read","documents:read","updates:read","usage:read"), rpm=100, max_top_k=5):
    return TenantRegistry([TenantPolicy.from_dict({"tenant_id":"test-tenant","name":"Test Tenant","enabled":True,"api_key_sha256":hash_api_key(KEY),"scopes":list(scopes),"allowed_document_ids":list(docs),"allowed_topics":["*"],"requests_per_minute":rpm,"max_top_k":max_top_k})])


def paths(tmp_path):
    p=ProductPaths.defaults(ROOT)
    return ProductPaths(real_records=p.real_records,fixture_records=p.fixture_records,real_published=p.real_published,lexical_config=p.lexical_config,vector_config=p.vector_config,hybrid_config=p.hybrid_config,tenant_config=tmp_path/"unused.json",usage_db=tmp_path/"usage.sqlite")


def client(tmp_path, *, mode="fixture", reg=None, limiter=None):
    ps=paths(tmp_path); ledger=UsageLedger(ps.usage_db)
    app=create_product_app(mode,paths=ps,tenant_registry=reg or registry(),canonical_publication_store=EmptyAuthority() if mode=="real" else None,usage_ledger=ledger,rate_limiter=limiter,allow_fixture=(mode=="fixture"))
    return TestClient(app),ledger


def headers(): return {"Authorization":f"Bearer {KEY}"}

def test_product_api_requires_auth(tmp_path):
    c,_=client(tmp_path); r=c.post("/v1/retrieve",json={"query":"Wanneer gebruik je de risicofactorenscore?"}); assert r.status_code==401; assert r.json()["detail"]["code"]=="missing_api_key"

def test_invalid_key_rejected(tmp_path):
    c,_=client(tmp_path); assert c.get("/v1/documents",headers={"Authorization":"Bearer wrong"}).status_code==401
...
def test_explicit_unauthorized_document_filter_is_403(tmp_path):
    c,_=client(tmp_path); r=c.post("/v1/retrieve",headers=headers(),json={"query":"score","filters":{"document_ids":["secret-doc"]}}); assert r.status_code==403; assert r.json()["detail"]["code"]=="document_not_entitled"

def test_knowledge_outside_entitlement_is_not_disclosed(tmp_path):
    c,_=client(tmp_path,reg=registry(docs=("another-document",))); assert c.get("/v1/knowledge/vvn-osteoporose-fractuurpreventie-2024-p015-score-01",headers=headers()).status_code==404

def test_top_k_tenant_limit(tmp_path):
    c,_=client(tmp_path,reg=registry(max_top_k=2)); r=c.post("/v1/retrieve",headers=headers(),json={"query":"fractuurrisico","top_k":3}); assert r.status_code==400; assert r.json()["detail"]["code"]=="top_k_exceeds_tenant_limit"

def test_scope_is_enforced(tmp_path):
    c,_=client(tmp_path,reg=registry(scopes=("retrieve",))); r=c.get("/v1/documents",headers=headers()); assert r.status_code==403; assert r.json()["detail"]["code"]=="scope_denied"

def test_usage_ledger_hashes_query_not_plaintext(tmp_path):
    c,ledger=client(tmp_path); query="Welke score geldt vanaf 60 jaar bij fractuurrisico?"; c.post("/v1/retrieve",headers=headers(),json={"query":query}); import sqlite3
    with sqlite3.connect(ledger.db_path) as con: row=con.execute("SELECT query_sha256 FROM api_usage WHERE endpoint='/v1/retrieve'").fetchone(); schema=' '.join(x[0] or '' for x in con.execute("SELECT sql FROM sqlite_master WHERE type='table'").fetchall())
    assert row and len(row[0])==64; assert query not in schema; assert query not in ledger.db_path.read_bytes().decode('latin1',errors='ignore')

def test_usage_summary(tmp_path):
    c,_=client(tmp_path); c.post("/v1/retrieve",headers=headers(),json={"query":"fractuurrisico"}); assert c.get("/v1/usage",headers=headers()).json()["requests"]==1

def test_documents_and_updates_are_tenant_scoped(tmp_path):
    c,_=client(tmp_path); docs=c.get("/v1/documents",headers=headers()).json()["documents"]; assert len(docs)==1 and docs[0]["document_id"]==DOC; updates=c.get("/v1/updates",headers=headers()).json()["updates"]; assert len(updates)==1; assert DOC in updates[0]["documents"]

def test_fixture_requires_explicit_allow_flag(tmp_path):
    ps=paths(tmp_path)
    try: create_product_app("fixture",paths=ps,tenant_registry=registry())
    except ValueError as exc: assert "fixture mode is disabled" in str(exc)
...
def test_rate_limit(tmp_path):
    ticks=[0.0]; limiter=SlidingWindowRateLimiter(now=lambda:ticks[0]); c,_=client(tmp_path,reg=registry(rpm=1),limiter=limiter); assert c.get("/v1/documents",headers=headers()).status_code==200; r=c.get("/v1/documents",headers=headers()); assert r.status_code==429; assert r.json()["detail"]["code"]=="rate_limit_exceeded"

def test_specific_topic_entitlement_does_not_include_untagged_records(tmp_path):
    reg=TenantRegistry([TenantPolicy.from_dict({"tenant_id":"topic-only","name":"Topic only","enabled":True,"api_key_sha256":hash_api_key(KEY),"scopes":["retrieve"],"allowed_document_ids":["*"],"allowed_topics":["nonexistent-topic"],"requests_per_minute":100,"max_top_k":5})]); c,_=client(tmp_path,reg=reg); data=c.post("/v1/retrieve",headers=headers(),json={"query":"fractuurrisico"}).json(); assert data["status"]=="abstain"; assert data["reason"]=="empty_published_corpus"
