...
def test_session_revoke_is_immediately_visible_to_other_instance(workflow_postgres: PostgresCanonicalConfig) -> None:
    first = PostgresWorkflowIdentityStore(workflow_postgres)
    second = PostgresWorkflowIdentityStore(workflow_postgres)
...
def test_same_object_concurrent_write_rejects_one_stale_writer(workflow_postgres: PostgresCanonicalConfig) -> None:
    uploader, reviewer = _seed_accounts(workflow_postgres)
    first = PostgresConcurrentWorkflowDocumentStore(workflow_postgres)
    second = PostgresConcurrentWorkflowDocumentStore(workflow_postgres)
    ...
    def write(store: PostgresConcurrentWorkflowDocumentStore, rows: list[dict[str, Any]]) -> str:
        barrier.wait(timeout=5)
        try:
...
def test_different_object_concurrent_edits_both_survive(workflow_postgres: PostgresCanonicalConfig) -> None:
    uploader, reviewer = _seed_accounts(workflow_postgres)
    first = PostgresConcurrentWorkflowDocumentStore(workflow_postgres)
    second = PostgresConcurrentWorkflowDocumentStore(workflow_postgres)
...
def test_concurrent_audit_creates_from_two_instances_both_survive(workflow_postgres: PostgresCanonicalConfig) -> None:
...
def test_concurrent_review_events_keep_one_linear_hash_chain(workflow_postgres: PostgresCanonicalConfig) -> None:
...
def test_independent_instances_do_not_lose_authorization_updates(workflow_postgres: PostgresCanonicalConfig) -> None:
    uploader, reviewer = _seed_accounts(workflow_postgres)
    documents = PostgresConcurrentWorkflowDocumentStore(workflow_postgres)
    for snapshot_id in ("snap-base", "snap-a", "snap-b"):
        documents.write_bundle(envelope=_envelope(snapshot_id, uploader, reviewer), objects=_objects(snapshot_id))

    first = PostgresWorkflowReviewStore(workflow_postgres)
    second = PostgresWorkflowReviewStore(workflow_postgres)
    first.replace_snapshot_bindings("snap-base", [_authorization("snap-base", "snap-base-a", reviewer)])
    barrier = threading.Barrier(2)

    def replace(store: PostgresWorkflowReviewStore, snapshot_id: str) -> str:
        barrier.wait(timeout=5)
        store.replace_snapshot_bindings(snapshot_id, [_authorization(snapshot_id, f"{snapshot_id}-a", reviewer)])
        return "ok"

    with ThreadPoolExecutor(max_workers=2) as pool:
        results = list(pool.map(lambda args: replace(*args), ((first, "snap-a"), (second, "snap-b"))))
