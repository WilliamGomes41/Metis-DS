...
def test_invented_spans_are_source_fidelity_failure() -> None:
    ...
    assert admitted["gate_result"] == GATE_BLOCKED
    assert "source_fidelity_failure" in admitted["reason_codes"]
    assert "span_not_in_source" in admitted["reason_codes"]
    assert any(
        code in admitted["reason_codes"]
...
def test_extracted_definition_carries_literal_type_evidence(tmp_path: Path) -> None:
    ...
    console = _console(tmp_path)
    accounts = _accounts(console)
    receipt = _ingest_richtlijn(console, accounts, data=html, filename="definitie.html", title="Definitie")
    definition = _find_by_text(
        console.snapshot_objects(receipt["snapshot_id"]),
        "Continentie is een klinisch onderwerp",
    )
    assert definition.get("proposed_object_type") == "definition"
    admission = _admission(definition)
    assert admission["type_evidence_spans"]
    assert "type_evidence_missing" not in admission["reason_codes"]
