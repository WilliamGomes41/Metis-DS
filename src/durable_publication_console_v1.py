...
class DurablePublicationConsole(ReviewClosureConsole):
    ...
    def _manifest_from_release(release: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "immutable_storage_locator": str(release["source_locator"]),
            "objects": deepcopy(release["objects"]),
        }
    ...
    def _projection_from_authority(self) -> list[dict[str, Any]]:
        ...
        projected, blocked = build_projection(
            [
                {
                    "knowledge_object": deepcopy(row["knowledge_object"]),
                    "publication": deepcopy(row["publication"]),
                }
                for row in authority_rows
            ]
        )
        if blocked:
            raise ConsoleError("durable_publication_projection_invalid", json.dumps(blocked, sort_keys=True))
        if len(projected) != len(authority_rows):
            raise ConsoleError("durable_publication_projection_incomplete")
        for projected_row, authority_row in zip(projected, authority_rows, strict=True):
            projected_row.setdefault("metadata", {})["snapshot_id"] = str(authority_row["snapshot_id"])
        return projected
    ...
    def _local_ledger_has_release(self, release_id: str) -> bool:
        ...
        for line in self._ledger_path.read_text(encoding="utf-8").splitlines():
            ...
            try:
                ...
                continue
            if (
                event.get("event_type") == "release_published"
                and str((event.get("details") or {}).get("release_id") or "") == release_id
            ):
                return True
        return False

    def _apply_local_release_copy(self, release: dict[str, Any], projection: list[dict[str, Any]]) -> None:
        """Apply rebuildable local publication state after durable commit.

        Failures roll back only local copies. The durable authority is never
        reversed because a JSONL/envelope/manifest/ledger write failed.
        """
        snapshot_id = str(release["snapshot_id"])
        release_id = str(release["release_id"])
        ...
        prior_manifest = self._read_optional(manifest_path)
        ...
        try:
            _atomic_write(manifest_path, self._manifest_from_release(release))
            atomic_replace_projection(projection_path, projection)
            ...
            current.update(
                {
                    ...
                    "release_version": str(release["release_version"]),
                    "published_at": str(release["published_at"]),
                    "published_by": str(release["release_owner"]),
                }
            )
            self._envelopes[snapshot_id] = current
            _atomic_write(self._envelopes_path, self._envelopes)
            if not self._local_ledger_has_release(release_id):
                append_event(
                    self._ledger_path,
                    event_type="release_published",
                    object_id=snapshot_id,
                    object_version=str(current.get("version") or ""),
                    actor=str(release["release_owner"]),
                    details={
                        "release_id": release_id,
                        "release_version": str(release["release_version"]),
                        "published_object_ids": sorted(
                            str(row["object_id"]) for row in release["objects"]
                        ),
                        "source_sha256": str(release["source_sha256"]),
                        "authority": "postgres",
                    },
            ...
            self._restore_optional(manifest_path, prior_manifest)
    ...
    def _durable_release_for_snapshot(self, snapshot_id: str) -> dict[str, Any] | None:
        ...
        if store is None:
            return None
        try:
            return store.release_for_snapshot(snapshot_id)
        except CanonicalPublicationStoreError as exc:
    ...
    def _sync_snapshot_from_authority(self, snapshot_id: str) -> dict[str, Any] | None:
        ...
        if release is None:
            return None
        projection = self._projection_from_authority()
        self._apply_local_release_copy(release, projection)
        return release
    ...
    def _publish_locked(self, *, actor_id: str, snapshot_id: str) -> dict[str, Any]:
        ...
        # If PostgreSQL already contains this snapshot, a previous durable commit
        # succeeded even if the process died before local copies were written.
        ...
        if existing is not None:
            return {
                ...
                "published_items": len(existing["objects"]),
                "g2": "PASS",
                "cutover": True,
        ...
        # Validate projectability before committing publication, but do not write
        # the API projection yet. The authoritative projection is re-read below.
        _candidate_projection, blocked = build_projection(
            [
                {
        ...
        # Publication happens here. Nothing under /home/data is made published
        ...
        try:
            store.persist_published_release(
                ...
                release_id=release_id,
                release_version=release_version,
                release_owner=str(account["username"]),
                published_at=published_at,
                objects=objects,
            )
        except CanonicalPublicationStoreError as exc:
        ...
        release = self._durable_release_for_snapshot(snapshot_id)
        if release is None or str(release.get("release_id") or "") != release_id:
            raise ConsoleError("durable_publication_commit_not_readable")

        # The Product API copy is derived from the committed authority, never from
        # objects/*.jsonl or a previous published_projection.jsonl.
        try:
            projection = self._projection_from_authority()
            self._apply_local_release_copy(release, projection)
        except Exception as exc:
            raise ConsoleError("durable_publication_local_copy_failed", str(exc)) from exc
        ...
        return {
            ...
            "cutover": True,
            "canonical_authority": "postgres",
            "local_projection": "derived",
        }

    def reconcile_durable_publications(self) -> dict[str, int]:
        """Rebuild local publication copies from PostgreSQL authority.

        The old direction (local manifest -> database) is intentionally removed.
        Local state cannot make knowledge published. At startup each known source
        snapshot is checked against the authority; durable releases repair their
        local envelope/manifest/ledger and the API projection is rebuilt from the
        ...
        for envelope in self.list_envelopes():
            checked += 1
            release = self._durable_release_for_snapshot(str(envelope["snapshot_id"]))
            if release is not None:
                releases.append(release)
        if not releases:
            # Even with no local envelope marked published, ensure stale API output
            # cannot survive when the durable registry is empty.
            atomic_replace_projection(self._published_projection_path(), self._projection_from_authority())
            return {"checked": checked, "reconciled": 0}

        projection = self._projection_from_authority()
        releases.sort(key=lambda row: str(row.get("published_at") or ""))
