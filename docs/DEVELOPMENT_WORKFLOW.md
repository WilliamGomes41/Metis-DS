...
Protocol v2.12 legt extractie als structuur/provenance only, de gesloten object-typeset met unclassified-default, answerability als vraagtype × objecttype, publish-binding aan het objecttupel, en serving vanuit een atomaire gepubliceerde projectie vast. Alle v2.6–v2.10-regels blijven van kracht, behalve de begrensde supersessie van extractie-betekenis, «alleen recommendations», envelope-`review_passes`, live-governance-per-query, en de eerstvolgende implementatie. De volgende implementatie is de Implementation engineer op de bestaande kernel; DAARNA G2/Azure. Geen Azure, geen Vercel/Neon, geen mockup; deze protocol-PR wijzigt `src/operations_console_*.py`, `src/extract_*.py` of `src/product_api_*.py` niet. Ingest URL-HTML blijft het v2.11-slot in open PR #27 (niet de live baseline). GD-03 wordt niet heropend.
