...
Index/conflict pointer: Protocol v2.32.0 SUPERSEDES any reading of this file, Protocol v2.10, or later deltas that the live console room heading MUST be Documentenhierarchie or Documentenhiërarchie. Where those files and Protocol v2.32 conflict on the live `/tree` heading / primary nav label, Protocol v2.32 governs: **Documenten**; MUST NOT Documentenhiërarchie, Documentenhierarchie, or Familieboom as the live heading; kernel remains family × class; v2.27 unpublished-delete remains one place — that same room, now labelled Documenten — plus type-to-confirm exact title; MUST NOT reopen delete to Review/Inleveren. v2.31 exact heading bind, HANDOFF.md MUST NOT be recreated, G2 remains BLOCKED, and `publish()` stays G2-BLOCKED remain.
