...
**Status:** Draft for technical and governance review
**Protocol version:** 2.2-draft.2
**Date:** 2026-08-20
...
|---|---|---|
| G1 Repository baseline | BLOCKED | `WilliamGomes41/VENVN-DS` with default branch `main` is the authoritative private remote and Step 12C was squash-merged as commit `1b7c068504432dc9799bf7364b264ef772851b00`; enforceable branch protection and required post-merge status checks are not available under the current private personal-account repository plan |
...
## 16. Governance decisions required before v2.2 approval

The following items require explicit governance approval and MUST not be hidden in implementation defaults:

1. Minimum independent holdout size and required high-risk composition.
2. Statistical reporting method and confidence level for FAR.
3. Required reviewer count for C3–C6 pull requests.
4. Maximum emergency-withdrawal time and retrospective break-glass review interval.
5. Supported API deprecation period.
6. Retention periods for acquisition records, audit logs, usage logs and confidential review evidence.
7. Named operational owner for production releases and emergency withdrawal.

## 17. Approval criteria for protocol v2.2

Protocol v2.2 MAY move from draft to approved only after:

- technical review confirms that every MUST requirement is testable or auditable;
- clinical governance confirms the acceptance and high-risk provisions;
- privacy/security review confirms repository, audit and query-handling requirements;
- the seven decisions in section 16 are resolved or assigned a formally recorded deadline and owner;
- the approved text receives an immutable version identifier and checksum.
