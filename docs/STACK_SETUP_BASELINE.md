...
The repository currently contains no frontend application and the Product API contains no LLM/generation layer. Vercel, Neon and a hosted LLM are therefore **not** current V&VN Data Services requirements.
...
| Canonical source binaries | Local file supplied to source registry; binaries excluded from Git | Controlled immutable source store. Repository conventions name **Azure Blob Storage** as target architecture | G2/G8 | **New durable storage cost** | Required before real source operation in Azure |
...
- `docs/PROTOCOL_V2_2.md` — G1/G8, source-storage and operational requirements;
