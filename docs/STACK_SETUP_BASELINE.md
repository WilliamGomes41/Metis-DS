...
| Canonical source binaries | Local file supplied to source registry; binaries excluded from Git. Local `sources/private/` is the G0 Local substitute and is explicitly not production. Azure Blob `aidataservice` / private container `canonical-sources` (West Europe) is the G2 store; locator `azure://aidataservice/canonical-sources/{sha256}/{filename}`; container empty as of 2026-08-31; G2 remains BLOCKED | Controlled immutable source store on **Azure Blob Storage**. Console ingest (Protocol v2.6) requires this store | G2/G8 / console ingest | Existing V&VN Azure subscription; storage cost after first source | Blob store provisioned empty; G2 BLOCKED; G0 Azure DEV remains BLOCKED |
