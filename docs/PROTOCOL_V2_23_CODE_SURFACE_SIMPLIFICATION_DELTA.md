...
**Highest change class:** C3 spanning review-surface / retrieve-safety (Auditor REQUEST SIMPLIFICATION after A+C+D on main; first DELETE cut, then one ZIP of that SHA; MUST NOT ZIP a566af56 if that forces a second ZIP)  
...
**Bounded supersession of any reading that (1) Cloud Shell ZIP of A+C+D SHA `a566af56` happens BEFORE the first DELETE cut; (2) two Cloud Shell ZIPs MAY be taken (ZIP of `a566af56` then a second ZIP after cleanup); (3) the first DELETE cut includes `semantic_transform_v2.py`, `prepublication_gate_v2.py`, `validation_workflow_v2.py`, `apply_second_review.py`, `canonical_store.py`, or `service_app.py`; (4) `service_app.py` MAY be deleted as a leftover; (5) PR #82 MAY be merged; or (6) this review is GD-03 or a publication decision.** Where this delta and those readings conflict, this delta governs. Owner lock 2026-09-03: first DELETE cut, then one ZIP of that SHA.
...
This is a **bounded supersession**. It does NOT reopen Protocol v2.11 freeze/locator, Protocol v2.12 types/projection, Protocol v2.13 atomic objects/relations/four-eyes, Protocol v2.15 ingest-date/version, Protocol v2.16 one-door / stacks / compact-rows / stamps / tiny-objects, Protocol v2.17 slogan / help / Onderwerp / bronpassage-prose / chrome / stamp-UI / relation-checkbox rules, Protocol v2.18 once-only card sentence / trailing-clause / identical-`clean_text` rules, Protocol v2.19 review duty / queue presentation, Protocol v2.20 every-guideline law / unpublished-snapshot delete, Protocol v2.21 wave A / wave B / wave C / wave D **definitions**, or Protocol v2.22 one-ZIP-then-B **except** the owner lock that the first DELETE cut MUST land on main BEFORE that one ZIP. It MAPS, and does NOT rewrite, existing law. Historical v2.21 wave definitions remain law. Protocol v2.22 one-ZIP-then-B remains law except this 2026-09-03 lock: first DELETE cut on main BEFORE any Cloud Shell ZIP; then ONE Cloud Shell ZIP of that resulting SHA; wave B still after that one ZIP + ingest. MUST NOT ZIP `a566af56` if that forces a second ZIP after cleanup. Two Cloud Shell ZIPs is refused. Protocol v2.16–v2.18 already forbid tiny objects, stamp-as-heading/object, truncated/trailing clauses, chrome objects, and identical `clean_text`. Protocol v2.19 is duty-queue. Protocol v2.20 unpublished-delete remains on main, not a fifth wave. The v2.12 closed serving typeset remains UNCHANGED:

`heading`, `definition`, `explanation`, `condition`, `exception`, `recommendation`

plus `unclassified` as default, not a sixth advice type. Operators MUST NOT invent types. Adding a type is a new protocol change. MUST NOT add new object types for page, paragraph, stamp, strength, GRADE, chrome, “light/heavy” review, wave, splitter, reject, or leftover. This delta’s bar is the first DELETE cut, then one ZIP of that SHA, not a new object type.
...
This delta also sets the next concrete implementation BEFORE the Cloud Shell ZIP. Protocol v2.22 §3 and §9 set the next implementation as wave C then wave D then ZIP then B. Waves C and D are already in code on `main` `a566af56c8c88e76cb4de7fa51642b408705da02`. Where this delta and Protocol v2.22 conflict on whether Cloud Shell ZIP of `a566af56` MAY happen BEFORE the first DELETE cut, this delta governs: it MUST NOT. The next **code** implementation MUST be one deletion PR on the existing kernel/repo (first DELETE cut plus integrity-sprint fixture retarget), existing pytest only, MUST NOT touch the splitter or console in that PR, on main BEFORE any Cloud Shell ZIP. Then ONE Cloud Shell ZIP of that resulting SHA (A+C+D already on main, plus v2.20 delete control, plus the first DELETE cut). MUST NOT ZIP `a566af56` if that forces a second ZIP after cleanup. Two Cloud Shell ZIPs is refused. Wave B still after that one ZIP + ingest. G2 still BLOCKED; `publish()` still G2-BLOCKED. ZIP does not open publication. Protocol v2.14 is still not written and is still not the next step. PR #82 stays closed/unmerged.
...
Wave A, wave B, wave C and wave D definitions in Protocol v2.21 remain law. Protocol v2.22 one-ZIP-then-B remains law except the owner lock that the first DELETE cut MUST land on main BEFORE that one ZIP. This delta does not rewrite those wave bodies and does not move wave B before that one ZIP. It records the Auditor first DELETE cut that MUST run on main before that ZIP, and the keep/do-not-delete list for that cut.

v2.20 unpublished-delete remains on main, not a fifth wave. `HANDOFF.md` MUST NOT be recreated.

## 3. Live path: first DELETE cut, then one ZIP of that SHA

Owner lock 2026-09-03 SUPERSEDES the earlier v2.23 claim that Cloud Shell ZIP of `a566af56` happens BEFORE the first DELETE cut.

Next live path:

1. First DELETE cut (the nine zero-caller src modules + integrity-sprint fixture retarget) happens on main BEFORE any Cloud Shell ZIP;
2. Then ONE Cloud Shell ZIP of that resulting SHA (A+C+D already on main, plus v2.20 delete control, plus the first DELETE cut);
3. then ingest a freeze;
4. then wave B (G2 evidence).

MUST NOT ZIP `a566af56` if that forces a second ZIP after cleanup. Two Cloud Shell ZIPs is refused. This protocol PR is not that ZIP. MUST NOT treat this protocol PR as Azure ZIP. MUST NOT Cloud Shell this protocol PR. PR #82 stays closed/unmerged. Waves C and D are already in code on `a566af56`. v2.20 delete control is already on main. Live still runs pre-wave-A extract until that one ZIP. Ingest of a new freeze MUST be after that one ZIP, not before. Wave B (G2 evidence/smoke) AFTER that one ZIP + ingest. G2 still BLOCKED. `publish()` still G2-BLOCKED. ZIP does not open publication.
...
Do not build a mockup. Do not wait for Azure, Vercel, Neon or a finished "DS" before the first DELETE cut on main.

Where this delta and Protocol v2.22 conflict on whether Cloud Shell ZIP of `a566af56` MAY happen BEFORE the first DELETE cut, this delta governs: first DELETE cut, then one ZIP of that SHA. Waves A, C and D are already in code on `main` `a566af56c8c88e76cb4de7fa51642b408705da02`. After this protocol merges:

1. Next code: one deletion PR, existing pytest only, first DELETE cut in section 6 plus integrity-sprint fixture retarget, MUST NOT touch the splitter or console in that PR. Implementation MUST re-prove no live callers before each delete. That cut happens on main BEFORE any Cloud Shell ZIP.
2. Then ONE Cloud Shell ZIP of that resulting SHA (A+C+D already on main, plus v2.20 delete control, plus the first DELETE cut). MUST NOT ZIP `a566af56` if that forces a second ZIP after cleanup. Two Cloud Shell ZIPs is refused.
3. Then ingest a freeze, then wave B (G2 evidence). G2 still BLOCKED; `publish()` still G2-BLOCKED. ZIP does not open publication.
...
- ZIP `a566af56` if that forces a second ZIP after cleanup;
- take two Cloud Shell ZIPs;
- ingest a new freeze before the one ZIP of the post-DELETE SHA;
...
- reopen Protocol v2.22 one-ZIP-then-B live path except as already required that the first DELETE cut MUST land on main BEFORE that one ZIP;
