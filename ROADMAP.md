...
Status: console-MVP ingest+review geïmplementeerd; console-UX-rewrite (Protocol v2.9 / PR #25) is nu in code op de bestaande kernel (taakgerichte onderzoeker-UX, V&VN digitale stylesheet, officieel beeldmerk). Protocol v2.6 keurt de scope goed; Protocol v2.10 eist Documentenhierarchie, wachttaak-badges en een Accounts-kamer — dat console-vervolg is in deze implementatiegolf in code. Protocol v2.12 objecttype/review/projectie is in dezelfde golf op de bestaande kernel. Protocol v2.11 blijft verplichte freeze/locator-wet (live URL-HTML weigeren; fail-closed `supported` zonder source locator) en is in deze golf kernel-code; het is geen supersessie van v2.12 §10. Geen mockup. Geen Azure. Geen Vercel/Neon. Publicatie blijft BLOCKED zonder immutable locator (G2). Duurzame opslag wordt niet overgeslagen.
