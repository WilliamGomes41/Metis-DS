...
- Een interne operations console MAG in deze repository als menselijk oppervlak over de knowledge kernel (Protocol v2.6, goedgekeurde scope; console-MVP ingest+review in code; Protocol v2.9-UX-rewrite in code; Protocol v2.10 eist Documentenhierarchie, wachttaak-badges en een Accounts-kamer). Een zorgapp-frontend, chatbot, EPD/ECD-UI of publieke website MAG dat niet. Chat hoort niet in de console.
...
Status: G1-technische protection ON; gezaghebbende remote is public onder Protocol v2.5.0; resterende nazorg: named GD-03-reviewers en retrospectieve C5/C3-review. Azure/G8 niet gestart. Protocol v2.11.0 is live. Protocol v2.10.0 blijft van kracht. Protocol v2.9.0 blijft van kracht. Protocol v2.8.0 blijft van kracht. Protocol v2.7.0 blijft van kracht, behalve de in v2.11 begrensde HTML-URL-ingestzin. Protocol v2.6.0 blijft de goedgekeurde console-scope; console-MVP ingest+review is geïmplementeerd; de v2.9-UX-rewrite (PR #25) is nu in code; het v2.10-consolevervolg (Documentenhierarchie, badges, Accounts) is niet geïmplementeerd; v2.11-ingestweigering en API-fail-closed zijn niet geïmplementeerd.
...
- Protocol v2.11.0 vastgesteld en live (v2.2 + v2.3-delta + v2.4-delta + v2.5-delta + v2.6-delta + v2.7-delta + v2.8-delta + v2.9-delta + v2.10-delta + v2.11-delta), inclusief G0, product-/distributiegrenzen, de MVP-uitzondering voor een publieke remote, de interne operations console als goedgekeurde scope, first-wave bron / retrieve-and-abstain / distributieregels, primaire gebruikers, klasse×familie-hiërarchie, de console-bouwvolgorde, de taakgerichte onderzoeker-UX plus V&VN digitale stylesheet, Documentenhierarchie, wachttaak-badges, de Accounts-kamer (gesloten rollenset), geüploade HTML-freeze, weigering van live URL-HTML, verplichte source locators en fail-closed Product API zonder locator. Protocol v2.10.0 blijft een geldend onderdeel van die baseline. Protocol v2.9.0 blijft een geldend onderdeel van die baseline. Protocol v2.8.0 blijft een geldend onderdeel van die baseline.
...
- Retrospectieve onafhankelijke review van de C5-wijzigingen in PR #4, PR #5, PR #16, Protocol v2.6 / PR #18, Protocol v2.7 / PR #19, Protocol v2.8 / PR #21, de console-implementatie / PR #23, de console-UX-rewrite / PR #25 en Protocol v2.10, plus C3-review van Protocol v2.9 en Protocol v2.11, blijft verschuldigd.
- G1 technische protection is ON. GitHub-ruleset **G1 main** (id `21686159`, 2026-08-27T22:10:53Z): geen verwijderen van `main`, geen force-push / non-fast-forward, required CI `test (3.12)` en `test (3.13)` (strict), pull request verplicht vóór merge, 0 vereiste goedkeurende reviews (solo owner). Protected branch, required CI en PR-workflow bestaan. Volgende uitvoeringsstap is de Implementation engineer op de bestaande kernel (Protocol v2.11): live URL-HTML bij ingest weigeren; Product API fail-closed `supported` zonder source locator. PROTOCOL → tests → code later. Deze protocol-PR implementeert die ingest-weigering en API-fail-closed niet. Het v2.10-console-vervolg (Documentenhierarchie, echte wachttaak-badges, Accounts-kamer) blijft verplicht; taakgerichte onderzoeker-UX, V&VN digitale stylesheet en officieel beeldmerk blijven verplicht; geen mockup, geen Azure, geen Vercel/Neon. Publicatie blijft BLOCKED zonder immutable locator (G2).

Stopvoorwaarde: C3–C6-merges vereisen de vastgestelde GD-03-matrix en onafhankelijke menselijke reviewers op dezelfde commit/snapshot; named reviewers zijn nog niet bezet. Bugfixes van bestaande protocolregels blijven toegestaan. Protocol v2.5 (PR #16), Protocol v2.6 (PR #18), Protocol v2.7 (PR #19), Protocol v2.8 (PR #21) en Protocol v2.10 zijn eigenaarsgoedgekeurde C5-delta's; Protocol v2.9 en Protocol v2.11 zijn eigenaarsgoedgekeurde C3-delta's; retrospectieve technical-, security/operations- en (voor v2.9 en v2.11) clinical-review van PR #4, PR #5, PR #16, v2.6, v2.7, v2.8, PR #23, v2.9, v2.10 en v2.11 blijft verschuldigd. GD-03 blijft ESTABLISHED.
...
### Fase 2b — Interne operations console (Protocol v2.6 / v2.8 / v2.9 / v2.10 / v2.11)

Status: console-MVP ingest+review geïmplementeerd; console-UX-rewrite (Protocol v2.9 / PR #25) is nu in code op de bestaande kernel (taakgerichte onderzoeker-UX, V&VN digitale stylesheet, officieel beeldmerk). Protocol v2.6 keurt de scope goed; Protocol v2.10 eist Documentenhierarchie, wachttaak-badges en een Accounts-kamer. Protocol v2.11 zet de volgende implementatie op de Implementation engineer op de bestaande kernel (live URL-HTML weigeren; fail-closed `supported` zonder source locator); het v2.10-console-vervolg blijft verplicht maar is niet de eerstvolgende implementatie. Geen mockup. Geen Azure. Geen Vercel/Neon. Publicatie blijft BLOCKED zonder immutable locator (G2). Duurzame opslag wordt niet overgeslagen.
...
- Protocol v2.11: officiële first-wave HTML MUST een geüploade freeze-file zijn. Live URL-HTML MUST bij ingest worden geweigerd. De Product API MUST NOT `supported` teruggeven zonder source locator. Deze protocol-PR implementeert die kernelwijziging niet. Het v2.10-console-vervolg blijft verplicht.
...
- Product API uitsluitend voeden met `supported` evidence op objectniveau. De Product API MUST NOT `supported` teruggeven als de source locator van het object ontbreekt of leeg is; fail-closed; abstain (cataloguszin, geen LLM) (Protocol v2.11). Deze protocol-PR implementeert die fail-closed niet.
